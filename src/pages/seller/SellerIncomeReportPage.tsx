        ...
        return (
          <div key={item.month} className="flex items-center gap-3">
            ...
            <span className="text-xs text-gray-700 w-28 shrink-0 text-right font-medium">
              {formatPrice(item.totalIncome)} ({item.totalOrders} order{item.totalOrders !== 1 ? 's' : ''})
            </span>
          </div>
        );
      })}
...
function IncomeByProductChart({ data }: { data: IncomeByProductItem[] }) {
  if (!data || data.length === 0) return null;
  const maxIncome = Math.max(...data.map((d) => Number(d.totalIncome)));
  if (maxIncome === 0) return null;
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-200 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">
            ...
            <th className="pb-2 pr-4 text-right">Gross Sales</th>
        ...
        <tbody>
          {sorted.map((item, idx) => (
            <tr key={idx} className="border-b border-gray-100 last:border-0">
              <td className="py-2.5 pr-4 text-gray-900 font-medium max-w-[200px] truncate">
                {item.productName || 'Unavailable product'}
              </td>
              <td className="py-2.5 pr-4 text-right text-gray-600">{item.quantity}</td>
              <td className="py-2.5 pr-4 text-right text-gray-900">{formatPrice(item.grossSales)}</td>
              <td className="py-2.5 text-right text-gray-900 font-medium">{formatPrice(item.totalIncome)}</td>
            </tr>
          ))}
        </tbody>
      </table>
        ...
        {hasOrders && (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
            ...
            {report.highestIncomeMonth && (
              <InsightCard title="Highest Month" value={report.highestIncomeMonth} />
            )}
          ...
          <>
            ...
            {report.incomeByProduct && report.incomeByProduct.length > 0 && (
              <Card header={<h2 className="text-lg font-semibold text-gray-900">Income by Product</h2>}>
                <IncomeByProductChart data={report.incomeByProduct} />
              </Card>
            )}

            {report.incomeByDeliveryMethod && report.incomeByDeliveryMethod.length > 0 && (
              <Card header={<h2 className="text-lg font-semibold text-gray-900">Income by Delivery Method</h2>}>
                <div className="grid sm:grid-cols-3 gap-3">
                  {report.incomeByDeliveryMethod.map((item) => (
                    <div
                    ...
                    >
                      ...
                      <p className="text-lg font-bold text-gray-900 mt-1">{formatPrice(item.totalIncome)}</p>
            ...
            {report.incomeByStatus && report.incomeByStatus.length > 0 && (
              <Card header={<h2 className="text-lg font-semibold text-gray-900">Income by Status</h2>}>
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {report.incomeByStatus.map((item) => (
                    <div
                      key={item.status}
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="text-sm font-bold text-gray-900">{formatPrice(item.totalIncome)}</p>
